...
**本地开发**：`npm run dev` 使用端口 **3001**（因本机 3000 已被 Supabase 占用），访问 http://localhost:3001 。

**服务器部署**：服务器上 3000 被 Supabase Studio（Docker）占用，Next 使用 **3001**；Nginx 反代到 `http://127.0.0.1:3001`。Jenkins 部署时通过 `APP_PORT=3001` 与 `ecosystem.config.cjs`（PORT=3001）启动 PM2。
