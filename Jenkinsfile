pipeline {
    ...
    stages {
        ...
        stage('Revalidate Next.js cache') {
            steps {
                echo '调用 Next.js /api/revalidate 刷新缓存...'
                sh '''
                  set -e

                  if [ -z "${REVALIDATE_SECRET:-}" ]; then
                    echo "REVALIDATE_SECRET 未配置，跳过 revalidate 调用"
                    exit 0
                  fi

                  JSON_PAYLOAD="{\\"secret\\":\\"${REVALIDATE_SECRET}\\",\\"paths\\":[\\"/\\"]}"

                  echo "===> 调用 https://next.sunyas.com/api/revalidate 刷新首页缓存..."
                  curl -s -X POST "https://next.sunyas.com/api/revalidate" \
                    -H "Content-Type: application/json" \
                    -d "${JSON_PAYLOAD}" || echo "revalidate 请求失败，请手动检查"
                '''
            }
        }
    }
