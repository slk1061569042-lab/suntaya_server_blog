  ...
  return (
    <div className="relative" ref={searchRef}>
      <div className="relative">
        ...
        />
        <svg
          className="absolute left-3 top-3 w-5 h-5 text-[#94A3B8]"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        <div className="absolute top-full mt-2 w-full glass-card rounded-xl shadow-2xl z-50 max-h-96 overflow-y-auto">
          {results.map((result) => (
            <Link
              ...
              className="block px-4 py-3 hover:bg-[#3B82F6]/10 transition-colors duration-200 border-b border-[#1E293B] last:border-0"
            >
              <div className="font-medium text-[#F1F5F9]">{result.title}</div>
              <div className="text-sm text-[#CBD5E1] mt-1">{result.description}</div>
